...
def main():
    ...
    subPropsExpectRateCol = common.getColBy2Para('初始副属性', '预期次数', dataList)
    subPropsNumCol = common.getColBy2Para('初始副属性', '属性数量', dataList)
    subPropsCol = common.getColBy2Para('初始副属性', ['副属性1', '副属性2', '副属性3', '副属性4'], dataList)
    ...
    for i in range(2, maxRow):
        ...
        if dataList[i][subPropsCol[0]] is not None:
            ...
            subPropsNumList.append(dataList[i][subPropsNumCol])
            tempList = []
            for j in range(4):
    ...
    for _ in range(tryTimes):
        ...
        for j in range(len(subPropsExpectRateList)):  # 是否满足属性组合
            for m in subPropsList[j]:
                ...
                if subPropsNumList[j] == len(hasList):
