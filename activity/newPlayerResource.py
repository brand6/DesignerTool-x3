...
def main():
    ...
    with WorkApp() as app:
        for i in range(lastCell.column):
            if dataList[0][i] is not None:
                wbName = dataList[0][i]
                printer.setStartTime("开始打开来源表格:" + dataList[0][i], 'green')
            ...
            if dataList[2][i] is not None:
                keyStr = dataList[2][i]
                if keyStr == 'key':
                    colOrder = common.getDataColOrder(shtData, keyStr, 2)
                    if colOrder != -1:
                        checkColMap[colKey].append(colOrder)
                        keyColMap[colKey].append(i)

            if dataList[3][i] is not None:
                colStr = dataList[3][i]
    ...
    for key in resultMap:
        dataSht.cells(4, key + 1).options(transpose=True).value = resultMap[key]
